...
function filterCodeBlock(str) {
  while (/<figure class="highlight ([a-zA-Z]+)">.*?<\/figure>/.test(str)) {
...
function generator(cfg, site) {
  ...
  // hooks after post rendered
  for (const post of site.posts.data) {
    post.excerpt = filterCodeBlock(post.excerpt)
    post.content = filterCodeBlock(post.content)
  }
